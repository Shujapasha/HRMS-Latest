...
class Utility extends Model
{
    ...
    public static function colorset()
    {
        ...
        if(\Auth::user())
        {
            ...
            $user = \Auth::user()->id;
            $setting = DB::table('hrm_settings')->where('created_by',$user)->pluck('value','name')->toArray();

        }
        else{

            $setting = DB::table('hrm_settings')->pluck('value','name')->toArray();
        }
    ...
    public static function get_superadmin_logo(){
        $is_dark_mode = DB::table('hrm_settings')->where('created_by', '1')->pluck('value','name')->toArray();
